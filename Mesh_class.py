...
class MeshGrid:
    ...
    def re_meshed(self, dis: float = 1.0):
        column_name = list(self.imported_mesh.columns)[-1]
        self.mesh = create_grid(self.imported_mesh['x'], self.imported_mesh['y'], self.imported_mesh[column_name], grid_size=1)
...
def file_to_work(filepath, dis = 1, to_dim = True, to_mkm = True, re_mesh = True, plot = False) -> MeshGrid:
    """
    Description:
    ...
    if plot == 'finish':
        ...
        axs[0].scatter(mesh.imported_mesh['x'], mesh.imported_mesh['y'], c = mesh.imported_mesh.iloc[:,-1])
